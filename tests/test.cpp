...
TEST(Json_parser_test, parse_right_file) {
    auto students = parse_json_file(RES_DIR"json_file1.json");
...
TEST(Json_parser_test, meta_error) {
    EXPECT_THROW(parse_json_file(RES_DIR"json_file2.json"), std::runtime_error);
}

TEST(Json_parser_test, items_not_an_array) {
    EXPECT_THROW(parse_json_file(RES_DIR"json_file3.json"), std::runtime_error);
}

TEST(Json_parser_test, wrong_path_error) {
    EXPECT_THROW(parse_json_file(RES_DIR"json_file.json"), std::runtime_error);
}
...
TEST(Json_parser_test, wrong_type_group) {
    EXPECT_THROW(parse_json_file(RES_DIR"json_file4.json"), std::invalid_argument);
}

TEST(Json_parser_test, wrong_type_avg) {
    EXPECT_THROW(parse_json_file(RES_DIR"json_file5.json"), std::invalid_argument);
}

TEST(Json_parser_test, wrong_type_debt) {
    EXPECT_THROW(parse_json_file(RES_DIR"json_file6.json"), std::invalid_argument);
}
